...
`STEAM_WEB_API_KEY` is configured and accepted, and the privacy route publishes `STEAM_DATA_STORAGE_COUNTRY=US`. A consenting member's profile and 259-game visible library synchronized successfully without creating Habitat XP. Steam returns HTTP 403 for player-achievement progress on both documented achievement hosts even though profile, library, and achievement-schema access succeeds; Habitat therefore labels and defers the entire achievement queue as access-restricted rather than spending requests across every app. Private-account UI behavior and production-scale call volumes remain pending. `MARVEL_RIVALS_API_KEY` remains unconfigured while that provider is unavailable, so Marvel pagination/retention and shared-match convergence also remain pending. Cross-game consumers are seeded disabled behind `HABITAT_CROSS_GAME_CONSUMERS_ENABLED=false` until those live fixtures and shadow comparisons are reviewed. Steam published game stats remain deferred because no allow-listed game-specific stat contract has been validated.
