...
- [x] Generated separate 1536 × 1024 clean and labeled world-map candidates from the approved V1 composition, with clearer shared-boundary-ready segmentation, preserved macro geography, Port Arcadia, the Floating City, and owner-confirmed Ignit Island placement
...
- [x] Generated one clean, unlabeled 1536 × 1024 production-refinement candidate from the V2 clean composition, preserving the established continent, regions, Grand Lake/Floating City, Port Arcadia, Ignit Island, far southwestern city-island, and southeastern volcanic archipelago
...
- [x] Preserved the lower-drift attempt as `martino-world-map-v2-topology-lock-review-candidate.png`; it consolidates Red Forest into one top-level footprint and provides a visible nested Death Canyon loop while keeping established anchors, including Ignit Island, in place
...
- [x] Kept active database writes, active migration applications, and seed runs at zero; Ignit Island and Port Arcadia remain world anchors/independent child scenes, and V1 renderer/API/routes/URLs/SSE/artwork/Bundle V4 remain active and untouched
...
- [x] Authenticated isolated QA passed on desktop and 390 x 844 mobile for clean rest state, hover, persistent selection, click-to-fit, nested Death Canyon, Grand Lake, Floating City, Ignit Island, and child-map navigation; V2 now derives `Ignit Island Tactical Atlas` from the owning canonical entry while preserving the stale V1 map title as compatibility data
...
- [x] Added scene-aware direct links and browser history for world selection, nested Death Canyon, Ignit Island, and Port Arcadia; Back/Forward and World breadcrumbs restore the expected scene and selection
- [x] Reviewed all 25 semantic connections in one deterministic backlog, persisted nine high-confidence paths (five Ignit roads, two world river corridors, one world sea route, one air route), left fourteen for review, and deferred two non-route transitions
...
- [x] Preserved legitimate hierarchy: Port Arcadia remains inside Peninsula with seven districts and five destinations, Death Canyon remains inside Grand Rift, Grand Lake and Floating City remain inside High Cliffs, all Bloomfall subregions/POIs remain attached, and Ignit Island/child-scene/progression structure remains unchanged
...
## 2026-08-28 - Bloomfall Reach character portrait publication

- [x] Remade Tomas Vey as an older, visibly alcohol-worn Southreach survivor while preserving his burn history, obsolete engineering kit, workshop setting, and mature photorealistic character direction
...
## 2026-08-31 - SEQ-000 owner rulings and Wendy portrait

- [x] Created `stormglass-recruitment-camp` as a CANON v1 Red Forest site with Wendy’s table at center, the wardrobe pod to the right, separate disposable two-person deployment pods to the left, no authored Atlas point, and no Soul Forge
- [x] Promoted `wendy` to CANON v2: she works before the island at the Red Forest camp, survives because she is not present for the Strike, returns later in the region, uses she/her, and carries an explicit casting/generation reference rather than an in-world ethnicity
- [x] Revised `steve` to CANON v11 as Wendy’s Blue Spiral guard and two-person pod escort; his prior leg and face injuries were real but completely grafted and healed, his exact background remains unknown, and his rooftop death, tutorial function, and Tino eulogy remain intact
- [x] Revised `enlistment` to CANON v3 and authored CANON scene `the-island-is-already-lost/seq-000` v1: wardrobe choices, visible service file, name/home/history/background/class/origin, Wendy’s “Brains Test” attribute screen, signature, Steve’s injection and final joke, and direct cinematic handoff are now one explicit sequence
- [x] Established the class allocation law without inventing the missing table: class determines the base numeric pool and placement; the player adds two flexible points and may move at most one class-allotted point; species ceilings remain hard
- [x] Separated background history from the eight class choices in `character-classes` CANON v5, established the known growth directions, and left exact base totals, starting skills, and cybernetics as explicit owner questions
- [x] Revised `transportation` CANON v3 and `stormglass-cartel` CANON v9 for the unreliable deployment pods and Steve’s Blue Spiral membership without claiming the whole crew owns the recruitment line
- [x] Inserted `seq-000 -> live-from-the-island` as the sole opening route and verified the mainline arc still has exactly one root; linked the scene to twelve exact Codex entries
- [x] Normalized every current player-facing Codex and story occurrence to “Hypogriff,” retained the immutable technical `hippogriff` slug/link, and synchronized the relevant seed and reauthor sources so the old display spelling cannot return on reseed
- [x] Applied the live package in one Serializable, optimistic-version-guarded transaction with exact fingerprints, active-lock checks, mixed-state refusal, audit revisions, and an idempotent zero-write second preview; an initial metadata-null attempt rolled back fully before the successful retry
- [x] Captured and byte-verified the successful target-only preimage at `N:\The Habitat\backups\codex-preimages\seq-000-owner-rulings-2026-08-31T19-58-16-835Z.json` (192,020 bytes; SHA-256 `e6add37e61981b54d5f11283041da7f3824912bea10be3694de84f3112b37b66`)
...
## 2026-09-02 - Nation and Trades pages in the Character page's grammar
...
- [x] Nation: `lib/nation.ts` holds Holding Ground as data - rungs and the verbs each adds, the four ways to get ground, the Nation Level law (1.6x curve, ceilings every third level), six realm trees, faiths with the campaign's morale numbers, the siege as a Forge clock with the measured storm/wait table and the soulless garrison, Court Day's four priced options, Syndicate, Mourning, standing laws - rendered through the new `components/field-card.tsx`; the live world board is unchanged
- [x] Typecheck, lint and 547 of 547 tests pass; screenshot-verified locally at desktop and mobile; deployed and pushed

## 2026-09-02 - SOL 5.6 The Crown Nation-art delivery

- [x] Generated and installed the exact 12-plate commission under `private/codex-art/nation`: one Map Table hero, five late-evening Ranks of the Crown, and six physical realm-tree sigils
- [x] Preserved the standing ruler viewpoint, blank-surface rule, restrained hand-worked brass, steady Forge infrastructure light, and every hard story lock; no god view, HUD, crossroads, throne gloss, heraldry, exposed First Weir bottom, active Outfall Anchor, Alder Wade memorial, Brother Aster figure, or anthropomorphic machine appears
- [x] Refused or superseded every visible count and thumbnail-readability miss: the hero advanced through six and four levers to the locked five; the Ward advanced from six to eight garrison; Court Day advanced from seven to exactly six seated officers plus its fatigued vassal; Might and Coffers were rebuilt after independent 64px review
- [x] Decoded-format verification passes 12/12 at the exact contracts: 1672x941 hero, 1200x800 ranks, and 1024x1024 sigils, all sRGB RGB24 PNGs with no alpha and twelve unique SHA-256 hashes
- [x] Corrected the realm-tree presentation to the commissioned 64x64 slot and pinned exact inventory, private-resolver round trips, decoded metadata, unique hashes, one reconciled ledger row per plate, page wiring, and CSS size in a 6/6 regression suite; the coverage audit reports `nation: 12/12 crown plates present`
...
- [x] Passed 23/23 focused art/resolver tests, full repository tests including 569/569 web tests, workspace typecheck, workspace lint with one pre-existing class-page image warning, the live Codex-art coverage audit, diff checks, and anonymous private-route checks; full-frame and crop QA plus exact prompts, sources, hashes, normalization, and revision evidence are reconciled in `Docs/art/SOL56_FAITH_ART_LEDGER.md` and retained local manifests
...
## 2026-09-02 - Nation terminology migration

- [x] Renamed the player-facing system, page, source modules, tests, simulation artifacts, art paths, metadata, seeds, and documentation to Nation/NM without changing the underlying ranks, progression, balance, or world-management mechanics
- [x] Made `/codex/nation` and `/codex/bible/nation-management` canonical while retaining permanent route compatibility for existing bookmarks and a shared route-to-storage alias for already-persisted StoryEntry rows
- [x] Projected legacy persisted StoryEntry copy and management-tier tags to Nation/NM at read time, including search aliases, so existing databases render the new terminology before any data migration
- [x] Added `scripts/migrate-nation-terminology.ts`: preview-only by default; applying requires an active ADMIN actor, an explicit confirmation token, a serializable transaction, optimistic version guards, and a StoryRevision before/after audit record
- [x] Kept `NATION_MANAGEMENT_PERSISTED_SLUG` on the pre-migration storage key for compatibility; new seed installs use Nation title/body/metadata and the compatibility key, while existing installations can use the audited migration path without a broken intermediate state
- [x] Audited the other live story text surfaces plus immutable StoryRevision and StoryRelease JSON. Frozen releases are never rewritten or hash-broken; after live canon is normalized, downstream game exports receive Nation wording only when an administrator deliberately cuts and promotes a new release
- [x] No database migration, external write, deployment, service restart, credential change, or server-control action was run as part of the source terminology change

## 2026-09-02 - SOL 5.6 The Blackweir Anaconda art delivery

- [x] Generated, revised, and installed the exact 25-plate commission: two figure anchors, three boss scenes, three recovered spoils, four encounter-mechanic tiles, six Phase One ability tiles, and seven Phase Two ability tiles
- [x] Preserved the canon and visual locks: clinical rather than anguished Vey, infrastructure-scale Phase Two with one tiny amber-lit adult, Blackweir continuity, localized contaminated-Essence light, intact ordered catalogue bodies, blank discs and fittings, and no readable text, brass, coin-warm gold, rainbow Essence, collage framing, or gratuitous gore
- [x] Decoded all finals at the exact contracts — eight 1672x941 plates and seventeen 256x256 tiles, all opaque non-palette sRGB RGB24 PNGs with 25 unique SHA-256 hashes — and reconciled every installed path, source, revision, byte count, and hash in `Docs/art/SOL56_ANACONDA_ART_LEDGER.md`
- [x] Reviewed all large masters, exact 564x941 center crops, the 1122x941 Phase One character crop, and every tile at 256px, commissioned 96px, and the live page's stricter 44px size; targeted reshoots corrected archetype drift, missing scale proof, clipped anatomy, and narrow-crop object loss
- [x] Added a seven-test exact-inventory regression suite covering authenticated resolver round trips, decoded metadata, unique hashes, ledger reconciliation, all three scene paths, all seventeen derived ability paths, and the live 44px CSS contract; the coverage audit reports `bosses: 20/20 mythic plates present`
