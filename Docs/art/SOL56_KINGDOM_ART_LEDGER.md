...
**Authority:** `Docs/art/SOL56_KINGDOM_ART_PROMPT.txt`

## Acceptance summary

- Installed the exact private convention inventory at `apps/web/private/codex-art/kingdom/<slug>.png`: one 1672x941 hero, five 1200x800 rank plates, and six 1024x1024 realm-tree sigils.
...
- Route gate: **PASS**. All twelve files round-trip through the authenticated private resolver; `audit-codex-art-coverage.ts` reports `kingdom: 12/12 crown plates present`.
- Authenticated browser boundary: this run's available browser had no local member session and correctly redirected `/codex/kingdom` to `/sign-in`, so no authenticated live-page result is claimed. Full-frame, rank-card, hero-cover, 64px, and 56px static layout simulations are retained locally instead.
...
- The issued commission is retained verbatim at `Docs/art/SOL56_KINGDOM_ART_PROMPT.txt`; the exact expanded prompts, attempt dispositions, generated-source paths, source hashes, final hashes, and decoded metadata are retained in:
...
- `pnpm exec tsx --test lib/kingdom-art.test.ts` from `apps/web`: **6/6 pass** — exact inventory, private resolver round trip, exact decoded dimensions/format, unique hashes, one reconciled ledger row per plate, all three page wiring paths, and the 64px CSS slot contract.
- `pnpm exec tsx scripts/audit-codex-art-coverage.ts` from `apps/web`: **Kingdom 12/12 present**. The audit's single remaining Codex gap is the unrelated pre-existing `FACTION/the-radiant-path` slot.
...
- Anonymous `GET /codex-art/kingdom/hero.png`: **HTTP 404**, preserving the private-art boundary.

## Publication boundary

The twelve files remain under `apps/web/private/`. They are served only by the existing authenticated `/codex-art` route and become visible on the next authenticated Kingdom-page reload. No StoryEntry, database row, public asset, credential, infrastructure address, server-control surface, or production service was changed.
