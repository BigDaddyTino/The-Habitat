...
- [x] Added three real Shame definitions: always-available `Most Verified Deaths` uses trusted hosted `PLAYER_DIED` evidence, while provider-derived `Most Heroic Defeats` and `Most Consolation Crowns` continue to honor the existing cross-game consumer rollout flag
