...
- [x] Verified the local database is current across all 43 migrations. Production inspection at `https://habitat.martinobear.com/admin/seasons` currently returns the Habitat 404 page, so this builder pass is correctly recorded as pending deployment rather than claimed live
- [x] Full workspace verification after the hardening pass: strict typecheck, lint, all 230 tests, production build, and `git diff --check` are green
