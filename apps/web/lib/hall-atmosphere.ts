...
export type HallEncounter = "raven" | "bear" | "ufo" | "comet" | "aurora" | "fireflies";
...
export function getGreatHallAtmosphere(now = new Date()) {
  ...
  const encountersBySky: Record<HallSky, HallEncounter[]> = {
    sunrise: ["raven", "bear", "raven", "ufo"],
    midday: ["raven", "bear", "ufo", "raven"],
    sunset: ["raven", "fireflies", "comet", "bear"],
    night: ["comet", "aurora", "ufo", "fireflies"],
  };
  const encounters = encountersBySky[sky];
