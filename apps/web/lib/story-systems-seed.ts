...
For writers: name the route, not just the destination. A broken-down truck is a quest; a guarded bridge is a faction statement; a safe road is somebody's ongoing expense. Where fast travel exists it should be diegetic — a paid convoy seat, a ferry schedule — with a price and a departure time.`,
  ...
  },
  {
    ...
    body: `Built for this crew from the start, and opened at a specific moment: the prologue is single-player, and CO-OP AVAILABLE fires at Kestrel the instant the tutorial completes — friends arrive in fiction rather than by teleport, as Stormglass survivors from another section of the island or soldiers already in camp. From there it is drop-in co-op in one persistent world. The ledger is shared — one island, one count of boats — while reputation keeps per-player texture inside a party-level standing. The big forks bind the group: Defend or Flee commits everyone, which makes the table itself a story pressure.
...
For writers: construction is a promise with a delivery date. Write quests around what a half-built thing invites — sabotage, squatters, a faction's sudden interest. Give materials provenance; whose timber, and what did it cost them? And write what the new structure displaced, because on a crowded island the answer is never nothing.`,
  ...
  },
  {
    slug: "professions",
    title: "Professions",
    summary: "Nine trades — Medicine, Refining, Chemistry, Engineering, Logistics, Architecture, Extraction, Culinary, Xenobiology — four rungs each and every one uphill, for players and companions who want to be needed for what they can make.",
    body: `Canon already reserves the medic, the mechanic, and the quartermaster in [[the-unnamed]] — professions make that party texture mechanical. Nine trades carry it, four rungs each: Medicine including corruption care, Engineering keeping rigs like the [[field-infusion-rig]] alive, and Refining — the moral ceiling of craft, where skill and complicity become the same thing. Companions carry professions, and settled ground needs them staffed.

**A trade levels by use, a licence gates every rung, and the grind is real.** The ladder is Apprentice → Licensed → Journeyman → Master, and nothing on it is handed out: doing the work raises the trade and nothing else does, only work at your current rung counts, a botched job feeds nothing, and reaching a count makes you eligible, never promoted — a person with the authority to refuse you signs, or you stay where you are. A journeyman journeys: three grounds' books signed before the rung opens, which is what carries the trades into every region of the world — including the unwritten ones, where seats are drawn and waiting — and never onto Ignit Island, which burns. Licensed in as many trades as you can keep busy; Master in exactly one, ever — except the Procurator, whose class carries the Second Seal and masters two, because the Procurator's whole life is licences. Trades are Earned, so death keeps them: your kit is on the corpse, your rungs are not.
...
**Dying abroad is not [[true-death]], and the two systems must not be confused.** An Incursor killed in a foreign Shard still has a living [[the-soul-forge]] holding their Soul Echo back home, and that Echo is untouched by the Crossing — it calls them home and they reclaim normally, out of their own Essence reserve. What they lose is everything they carried, which stays in the invaded Shard. The [[dimensional-echo]] a defender harvests is *not* that Soul Echo: it is the foreign resonance bleeding off — the mismatch between a person and a reality that was never theirs — and it is why Echoes drop only from outsiders. A crew that crosses unbound, though, is crossing with nothing to call them back, and that is true death like anywhere else.
...
Before the party's first Forge, the tutorial runs on **checkpoints**: die and you resume at the last one. That is scaffolding, and the game says so by taking it away — the prologue's TUTORIAL COMPLETE card is the last moment checkpoints exist, binding follows within minutes, and the difference should be felt.
...
**Where it happens first — Kestrel, and exactly when.** The prologue board already fixes the order, and this scene slots into it rather than over it. On arrival [[the-kestrel-commander]] asks about the partner the player cannot account for and gets no answer she can use. The title sequence runs. Then TUTORIAL COMPLETE, CO-OP AVAILABLE, and a few unforced minutes of camp. **When Rook calls the party over, her first question is not about the battle: *where are you bound?*** The answers characterise them and she reads each one — deflection ("what's it to you?") earns open annoyance; *"I don't know"* earns something closer to alarm, because to Rook that is a person walking a front line with nowhere to come back to. Then she walks them to the camp's Forge and binds them herself, on the reasoning that anyone who might stay and hold this island is no use to her unbound. Only then does she take them to the operations table. The scene teaches the mechanic through a character with obvious reasons to care, per [[the-war-teaches]].

**And it is where the party works out that Tino is not dead.** Rook asked where his partner was and got nothing; now the Core answers the same question a different way and gives them just as little. He is not at Kestrel, and no Echo of his sits here — so whatever happened, he did not reclaim here. The inference is that he is bound somewhere else. That is all it is: an inference. Binding on every writer, per [[what-the-player-knows-about-tino]] — **a Forge can only speak about Echoes bound to it.** Kestrel's Forge cannot confirm that Tino is alive, cannot confirm he is dead, and cannot be asked. Never write a scene where an instrument settles his fate; the empty Core is the point, and the party leaving with a question is the correct outcome. A party that raises him here should set [[asked-about-tino]].
