...
import type { AtlasV2Projection } from "@habitat/shared";
...
export function createStoryAtlasV2ProjectionService(client: AtlasPersistenceClient) {
  ...
  return async function project(slug: string): Promise<AtlasV2Projection | null> {
    ...
    const [connections, revision, sceneOwner] = await Promise.all([
      client.storyWorldConnection.findMany({ orderBy: { id: "asc" }, include: { fromEntry: { select: { slug: true } }, toEntry: { select: { slug: true } }, _count: { select: { paths: true } } } }),
    ...
    return buildStoryAtlasV2Projection({ v1, topology: topology.dataset, revisionCursor: revision?.id ?? null, sceneOwnerTitle: sceneOwner?.owner?.title, connections: connections.map((connection) => ({ id: connection.id, fromSlug: connection.fromEntry.slug, toSlug: connection.toEntry.slug, type: connection.type, directionality: connection.directionality, status: connection.status, visibility: connection.visibility, hasPath: connection._count.paths > 0 })) });
  };
}
