...
import { existingRaceSheets, raceAssignments, raceSeeds } from "./story-races-seed";
...
test("race prose links only ever reach for well-formed slugs", () => {
  ...
  const humans = raceSeeds.find((seed) => seed.slug === "humans");
  assert.ok(storyProseLinks(humans?.body ?? "").includes("the-seven-phases-of-corruption"));
});
