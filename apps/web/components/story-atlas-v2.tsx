...
export function StoryAtlasV2({ initialProjection }: { initialProjection: AtlasV2Projection }) {
  ...
  const select = useCallback((slug: string | null, fit = false) => {
    selectedRef.current = slug;
    setSelectedSlug(slug);
    if (typeof window !== "undefined") window.history.replaceState(null, "", slug ? atlasV2Hash(slug) : `${window.location.pathname}${window.location.search}`);
  ...
  const openScene = useCallback(async (slug: string) => {
    if (slug === projection.scene.slug || loadingScene) return;
    setLoadingScene(true);
    try {
      const response = await fetch(`/api/codex/maps/${slug}?atlas=v2`, { cache: "no-store" });
      if (response.ok) { select(null); setQuery(""); setProjection(await response.json() as AtlasV2Projection); }
    } finally { setLoadingScene(false); }
  }, [loadingScene, projection.scene.slug, select]);
  ...
  useEffect(() => {
    ...
    const map = new OlMap({ target: targetRef.current, layers: [baseLayer, hitLayer, highlightLayer, pointLayer, labelLayer], view, controls: [] });
    mapRef.current = map; hitLayerRef.current = hitLayer; highlightLayerRef.current = highlightLayer; labelLayerRef.current = labelLayer; pointLayerRef.current = pointLayer;
    ...
    return () => { window.cancelAnimationFrame(frame); map.setTarget(undefined); mapRef.current = null; hitLayerRef.current = null; highlightLayerRef.current = null; labelLayerRef.current = null; pointLayerRef.current = null; };
  }, [sceneKey, select]);
  ...
  useEffect(() => {
    ...
    if (!regionSource || !labelSource || !pointSource) return;
    replaceProjectionFeatures({ projection, height: projection.scene.coordinateHeight, regionSource, labelSource, pointSource });
  ...
  useEffect(() => { selectedRef.current = selectedSlug; highlightLayerRef.current?.changed(); labelLayerRef.current?.changed(); pointLayerRef.current?.changed(); }, [selectedSlug]);
  useEffect(() => { const onHash = () => select(hashSelection(), true); window.addEventListener("hashchange", onHash); return () => window.removeEventListener("hashchange", onHash); }, [select]);
  useEffect(() => {
  ...
  const resetWorld = () => { select(null); const scene = projection.scene; mapRef.current?.getView().fit([0, 0, scene.coordinateWidth, scene.coordinateHeight], { padding: [26, 26, 26, 26], duration: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? 0 : 420 }); };
  return <section className="atlas-shell atlas-v2-shell" aria-label="Internal Atlas V2 projection">
    ...
    <div className="atlas-stage">
      <div className="atlas-toolbar"><nav className="atlas-breadcrumb" aria-label="Atlas location"><button onClick={resetWorld}>World</button>{breadcrumbs.map((item) => <button key={item.slug} onClick={() => focus(item)}>› {item.title}</button>)}</nav><label className="atlas-search"><Search aria-hidden="true" size={16}/><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && matches[0]) focus(matches[0]); }} placeholder="Find a region, anchor, or quest" aria-label="Search Atlas V2"/></label>{matches.length ? <div className="atlas-search-results">{matches.map((item) => <button key={item.slug} onClick={() => focus(item)}><span>{item.title}</span><small>{"role" in item ? item.role.toLowerCase().replaceAll("_", " ") : item.layer.toLowerCase()}</small></button>)}</div> : null}</div>
