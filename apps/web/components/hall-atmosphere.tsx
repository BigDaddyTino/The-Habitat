...
export function HallAtmosphere({ sky, encounter }: { sky: HallSky; encounter: HallEncounter }) {
  ...
  useEffect(() => {
    const hero = root.current?.closest<HTMLElement>(".hall-hero");
    if (!hero || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    const move = (event: PointerEvent) => {
      const bounds = hero.getBoundingClientRect();
      hero.style.setProperty("--hall-x", String((event.clientX - bounds.left) / bounds.width - .5));
      hero.style.setProperty("--hall-y", String((event.clientY - bounds.top) / bounds.height - .5));
    };
    const leave = () => { hero.style.setProperty("--hall-x", "0"); hero.style.setProperty("--hall-y", "0"); };
    hero.addEventListener("pointermove", move);
    hero.addEventListener("pointerleave", leave);
    return () => { hero.removeEventListener("pointermove", move); hero.removeEventListener("pointerleave", leave); };
  }, []);
  return <div ref={root} className={`hall-atmosphere sky-${sky} encounter-${encounter}`} style={{ mixBlendMode: "normal", zIndex: 1 }} aria-hidden="true">
