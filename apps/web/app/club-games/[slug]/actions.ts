...
async function saveSnapshot(profileId: string, profile: MarvelRivalsProfileData, source: string) {
...
export async function linkMarvelRivalsProfile(_previous: RivalsLinkState, formData: FormData): Promise<RivalsLinkState> {
  ...
  const provider: MarvelRivalsProvider | null = resolveMarvelRivalsProvider(process.env);
  if (!provider) return { status: "error", message: "Profile linking is not online yet. An administrator must configure a stats provider." };
  ...
  try {
    ...
    const profile = await db.clubGameProfile.upsert({
      ...
      create: { userId: user.id, gameType: "MARVEL_RIVALS", platform, displayPublic: false, ...profileFields(result, provider.profileRefreshIntervalMs) },
      update: { platform, ...profileFields(result, provider.profileRefreshIntervalMs) },
    });
    await Promise.all([
      saveSnapshot(profile.id, result, provider.source),
...
export async function refreshMarvelRivalsProfile(): Promise<void> {
  ...
  try {
    ...
    await db.clubGameProfile.update({ where: { id: current.id }, data: profileFields(result, provider.profileRefreshIntervalMs) });
    await saveSnapshot(current.id, result, provider.source);
  } catch (error) {
