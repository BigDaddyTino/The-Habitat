...
export async function requestServerCommand(formData: FormData) {
  ...
  const server = await db.gameServer.findUnique({ where: { id: parsed.data.serverId }, select: { id: true, displayName: true, controlEnabled: true } });
  ...
  await db.$transaction(async (transaction) => {
    ...
    await transaction.auditLog.create({ data: { actorUserId: actor.id, action: "SERVER_COMMAND_AUTHORIZED", entityType: "ServerCommand", entityId: command.id, after: { serverId: server.id, serverName: server.displayName, action: parsed.data.action } } });
  });
