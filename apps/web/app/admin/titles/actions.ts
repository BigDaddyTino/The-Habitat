...
export async function createTitleDefinition(formData: FormData) {
  ...
  if (!parsed.success) throw refusal("Invalid title definition.");
  const slug = parsed.data.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
  if (!slug) throw refusal("Title name must contain letters or numbers.");
...
export async function grantTitle(formData: FormData) {
  ...
  if (!parsed.success) throw refusal("Invalid title grant.");
  ...
  if (!title?.enabled || !recipient?.isActive) throw refusal("Title or recipient is unavailable.");
...
export async function updateTitleDefinition(formData: FormData) {
  ...
  if (!parsed.success) throw refusal("Invalid title update.");
  ...
  if (!existing) throw refusal("Title definition was not found.");
