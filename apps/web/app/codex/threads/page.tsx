 ...
 */
export default async function StoryThreadsPage() {
  await requireRole(storyReadRole);
  const ledger = await getStoryThreads();
  ...
  return (
    <section className="page-shell codex-shell">
      ...
      <div className="page-intro">
        ...
        <p className="eyebrow">Martino — story threads</p>
        <h1>Every promise the story has made</h1>
        <p>
          A thread is a flag: one quest sets it, another answers it — sometimes chapters apart. This page derives all of them from the
      ...
      {ledger.threads.length === 0 ? (
        <div className="empty-data"><Flag aria-hidden="true" size={24} /><div><h2>No threads yet.</h2><p>Create a FLAG entry in the bible and set it from a scene&apos;s effects — it will appear here with everywhere it&apos;s touched.</p></div></div>
      ) : (
      ...
      <p className="thread-footnote">
        To open a new thread: add a FLAG entry in <Link href="/codex/bible">the bible</Link>, then write <code>set flag: its-slug</code> in the effects of the scene or choice
