...
import { LEGACY_NATION_MANAGEMENT_GAME_TAG_PREFIX, NATION_MANAGEMENT_GAME_TAG_PREFIX, NATION_MANAGEMENT_ROUTE_SLUG } from "@habitat/shared";
import { FieldCard } from "@/components/field-card";
import { requireRole } from "@/lib/authorization";
import { codexArtSlot, findCodexArt } from "@/lib/codex-art";
import { courtDay, courtSeats, crownRanks, faithLaw, faiths, groundVerbs, machines, nationLevel, nationLevels, plotsLaw, provings, ranksLaw, realmPoints, realmTrees, realmTreesLaw, riverlandsPlots, sacredLaw, siegeLaw, standingLaws, succession, syndicate } from "@/lib/nation";
import { storyReadRole } from "@/lib/story-codex";
import "../play.css";
import "./nation.css";

export const metadata = { title: "Nation Management | Story Codex" };
export const dynamic = "force-dynamic";

/**
 * The Nation page: the crown read top to bottom the way a ruler climbs it.
 ...
 * `lib/nation.ts`. Art: private/codex-art/nation/<slug>.png, picked up on
...
function tierOf(gameTag: unknown): { key: TierKey; label: string } {
  ...
  const prefix = [NATION_MANAGEMENT_GAME_TAG_PREFIX, LEGACY_NATION_MANAGEMENT_GAME_TAG_PREFIX].find((candidate) => tag.startsWith(`${candidate} `));
  const label = prefix ? tag.slice(prefix.length).trimStart() : tag;
  ...
  if (label.startsWith("feeds")) return { key: "wing", label };
...
function Art({ slug, className, glyph }: { slug: string; className: string; glyph: string }) {
  const url = findCodexArt("nation", slug);
  if (url) {
  ...
  return (
    <figure className={className}>
      <span aria-hidden="true" className="nm-glyph">{glyph}</span>
      <figcaption className="play-artslot">art slot — Sol · <code>{codexArtSlot("nation", slug)}</code></figcaption>
    </figure>
  );
}
...
export default async function NationPage() {
  ...
  const tier = (key: TierKey, heading: string, note: string) => {
    ...
    return (
      <section className={`nm-tier is-${key}`}>
        <h3>{heading}</h3>
        <p>{note}</p>
        <div className="nm-powers">
          {rows.map((faction) => {
            const meta = faction.meta as Record<string, unknown>;
            const wings = childrenOf.get(faction.slug) ?? [];
            const faith = typeof meta.faith === "string" ? meta.faith : null;
            return (
              <article className={`nm-power is-${key}`} key={faction.slug}>
    ...
    <section className="page-shell codex-shell play-shell nm-shell">
      ...
      <header className="nm-hero">
        <div className="nm-hero-copy">
          ...
          <ul className="nm-hero-stats">
            <li><b>{crownRanks.length}</b><span>Ranks of the Crown</span></li>
            <li><b>{nationLevels.length}</b><span>Nation Levels</span></li>
        ...
        <Art className="nm-hero-art" glyph="♛" slug="hero" />
      </header>

      <div className="play-gamer">
        <b>In gamer terms</b>
        <span>Late game. Your realm has a <b>Nation Level, 1 to 15</b>; every three levels is a <b>Rank</b> (Freeholder → Warden → Magistrate → Lord → Crown) and every rank-up is a <b>proving quest</b>, not an XP bar. Each level grants a perk and extends your caps; each level and proving gives <b>realm points</b> to spend on <b>six talent trees</b>. Get ground by <b>buying, seizing, earning or founding</b> it; pick a <b>faith</b> that pays and costs; fight sieges as <b>Forge clocks</b>; hold <b>Court Day</b> monthly; run it with friends as a <b>Syndicate</b>; and when you die for good, your heir holds it or it fractures.</span>
      </div>
      ...
      <div className="play-jump">
        ...
        <Link href={`/codex/bible/${NATION_MANAGEMENT_ROUTE_SLUG}`}>Nation Management (canon)</Link>
      </div>

      {/* ------------------------------------------------------------- ranks */}
      <section className="play-law nm-ranks" id="ranks">
        <h2>The Ranks of the Crown: five ranks, fifteen levels</h2>
        <p className="play-lede">{ranksLaw}</p>
        <ol className="nm-stair">
          {crownRanks.map((rank, index) => {
            const proving = provingAfter.get(rank.levels[1]);
            return [
              <li className={`nm-rank is-rank-${rank.numeral.toLowerCase()}${index === crownRanks.length - 1 ? " is-crown" : ""}`} key={rank.title}>
                <Art className="nm-rank-art" glyph={rank.numeral} slug={rankArtSlug(rank)} />
                <header>
              ...
              proving ? (
                <li className="nm-gate" key={proving.name}>
        ...
        <p className="nm-foot"><b>The climb:</b> {nationLevel.curve} {nationLevel.ceilings} {nationLevel.firstCeiling} <b>Tall or wide:</b> {nationLevel.tallVsWide}</p>
      </section>

      {/* ------------------------------------------------------------ ledger */}
      <section className="play-law nm-ledger-law" id="ledger">
        ...
        <p className="play-lede">XP comes from real work only: {nationLevel.xpFrom.join("; ")}. Every level grants a perk, extends the caps, and pays <b>{realmPoints.perLevel} realm point</b>; every proving pays <b>{realmPoints.perProving} more</b>.</p>
        <div className="nm-ledger-scroll">
          <table className="nm-ledger">
            <thead>
              ...
              {nationLevels.map((row) => {
                ...
                return [
                  <tr className={`is-rank-${row.rank.toLowerCase()}${proving ? " is-ceiling" : ""}${row.level === rank.levels[0] ? " is-first" : ""}`} key={row.level}>
                    <td className="nm-lv">{row.level}</td>
                    <td className="nm-rk"><i>{row.rank}</i>{row.level === rank.levels[0] ? <span>{rank.title}</span> : null}</td>
                    <td className="nm-perk">{row.perk}</td>
                    <td className="nm-grants">{row.grants}</td>
                    <td>{row.caps.holdings}</td>
                    <td>{row.caps.muster}</td>
                    <td>{row.caps.seats}</td>
                    <td>{row.caps.vassals}</td>
                    <td className="nm-pts">+{realmPoints.perLevel}</td>
                  </tr>,
                  proving ? (
                    <tr className="nm-proving" key={`proving-${row.level}`}>
                      <td colSpan={9}>
        ...
        <p className="nm-foot"><b>Caps:</b> {nationLevel.capsNote} <b>Teacher:</b> {nationLevel.teacher}</p>
      </section>
      ...
      <section className="play-law nm-trees-law" id="trees">
        ...
        <div className="nm-trees">
          {realmTrees.map((tree) => (
            <article className={`nm-tree is-${tree.slug}`} key={tree.slug}>
              <header>
                <Art className="nm-sigil" glyph={tree.name.slice(0, 1)} slug={`tree-${tree.slug}`} />
                <div>
                  <b>{tree.name}</b>
                  <span>{tree.buys}</span>
                </div>
              </header>
              <ol className="nm-nodes">
                {tree.nodes.map((node) => (
                  <li className={node.capstone ? "is-capstone" : undefined} key={node.id}>
                    <i className="nm-cost" title={`${node.cost} realm ${node.cost === 1 ? "point" : "points"}`}>{node.cost}</i>
                    <div>
      ...
      <section className="play-law" id="ground">
        ...
        <p className="nm-foot"><b>Seizing the sacred:</b> {sacredLaw}</p>
        <h3 className="nm-sub">Plots: where you build your own buildings</h3>
        ...
        <p className="nm-foot"><b>Other regions:</b> their plots are drawn where their writing is. A region with no plot is a region where you hold ground by seizing, earning or founding it, not by buying.</p>
      </section>
              ...
              <FieldCard
                ...
                fields={[
                  ...
                  { label: "Morale", value: faith.morale, tone: "muted" },
                  {
                    label: "Kept by",
                    value: faith.secular
                      ? <span className="nm-keepers">{secularCount} of {factions.length} powers on the roster declare no faith; three of the five Great Powers among them.</span>
                      : keepers.length
                        ? <span className="nm-keepers">{keepers.map((keeper, index) => <span key={keeper.slug}>{index ? " · " : ""}<Link href={`/codex/bible/${keeper.slug}`}>{keeper.title}</Link></span>)}</span>
                        : <span className="nm-keepers is-empty">No power on the roster keeps it yet.</span>,
                  },
                  ...(ground.length ? [{
                    label: "Ground",
                    value: <span className="nm-keepers">{ground.map((row, index) => <span key={row.region} title={row.note}>{index ? " · " : ""}<Link href={`/codex/bible/${row.region}`}>{regionTitle.get(row.region) ?? row.region}</Link></span>)}</span>,
                    tone: "muted" as const,
                  }] : []),
                ]}
                key={faith.slug}
      ...
      {/* ------------------------------------------------------------- board */}
      <section className="nm-board" id="board">
      ...
      <section className="play-law" id="laws">
        <h2>Standing laws</h2>
        <ul className="nm-laws">{standingLaws.map((law) => <li key={law}>{law}</li>)}</ul>
      </section>

      <p className="nm-foot">
        The full law lives in the codex: <Link href={`/codex/bible/${NATION_MANAGEMENT_ROUTE_SLUG}`}>Nation Management</Link> ·{" "}
