...
export async function votePoll(formData: FormData) {
  ...
  if (!parsed.success) throw new Error("Invalid poll vote.");
  await db.$transaction(async (transaction) => {
    const poll = await transaction.serverPoll.findUnique({ where: { id: parsed.data.pollId }, include: { options: { select: { id: true } } } });
    if (!poll || poll.status !== "ACTIVE" || poll.closesAt <= new Date() || !poll.options.some((option) => option.id === parsed.data.optionId)) throw new Error("This poll is no longer open.");
