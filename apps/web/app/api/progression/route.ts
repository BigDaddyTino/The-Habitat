...
export async function GET() {
  ...
  const [total, entries, achievements] = await Promise.all([
    ...
    db.playerAchievement.findMany({ where: { userId: session.user.id }, orderBy: [{ awardedAt: "desc" }, { id: "desc" }], take: 16, select: { id: true, awardedAt: true, achievement: { select: { name: true, rarity: true, description: true, category: true, points: true, rewards: { select: { kind: true, code: true, name: true } } } } } }),
  ]);
