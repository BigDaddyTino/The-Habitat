  ...
  return (
    <section className="page-shell profile-page">
      ...
      <div className="profile-customizer">
        <form action={updateProfile} className="server-editor profile-form"><label>Callsign<input name="username" defaultValue={member.username ?? ""} placeholder="your-callsign" required /></label><label>Display name<input name="displayName" defaultValue={member.displayName ?? member.name ?? ""} placeholder="How the lodge knows you" required /></label><label className="field-wide">Bio<textarea name="bio" defaultValue={member.bio ?? ""} placeholder="Tell the lodge a little about your preferred brand of chaos." rows={4} /></label><button className="save-server" type="submit">Save profile</button></form>
      ...
      {identities.length === 0 ? <div className="chronicle-empty"><p>No verified identities yet.</p><span>Submit a claim once your game identity has been observed by a supported adapter.</span></div> : <div className="identity-grid">{identities.map((identity) => <article className="identity-card" key={identity.id}><p className="eyebrow">{identity.server?.displayName ?? identity.gameType.replaceAll("_", " ")}</p><h2>{identity.displayName}</h2><p>{identity.server?.worldName ?? "Habitat identity"}</p><span>{identity._count.events} recorded events</span></article>)}</div>}
