...
export default async function ProfilePage({ searchParams }: { searchParams: ProfileSearchParams }) {
  ...
  return <section className="page-shell profile-page">
    ...
    {/* The provider verification links below are plain anchors, not next/link.
        Both routes answer with a redirect to an external origin (Steam OpenID,
        Twitch OAuth), and the App Router's client-side navigation cannot follow a
        cross-origin redirect, so a Link click looks like a dead button. Link also
        prefetches, which silently spends a link nonce per hover. */}
