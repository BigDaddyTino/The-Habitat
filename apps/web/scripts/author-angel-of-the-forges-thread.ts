...
**Key art: in (2026-09-09).** The owner accepted a render and it sits in the thread's art slot (\`private/codex-art/threads/the-angel-of-the-forges.png\`): golden hair, mirror-plate armour with the deep V, the fractured-mirror wings full of faces, a marble city in living flowers and ivy, the drowned island green under the sea. Her eyes and her stone are **violet**, both with fine blue veins running through them, slightly illuminated, like stormglass — the owner confirmed it from this render on 2026-09-09 (the sheet had said red before; every mention is now violet).
...
One scene, one woman, one minute, three ways it goes. All three share the same stage — the platform at the edge of the Floating City before dawn, the drowned island glowing under the sea below — and the same Illyria as the key art above (golden hair, violet irises with faint lit blue veins, olive skin, mirror-plate armour with the deep V, the violet stone at her chest, fractured-mirror wings full of faces). **The player is never given a face.** The player is a figure seen from behind or in silhouette: a hooded travelling coat, a pack, a weapon on the back, so any player can be that shape. No board may use these; they are for the Idea Center and the owner.

> **GIVE.** Same stage, same woman, tender and quiet. Illyria stands close to the player, who is seen from behind in a hooded travelling coat, close enough that her mirror wings curve around both of them like a room. Her right hand is out, palm up, and resting in it is a small sphere of soft white light — the player's Echo — lifting from the player's chest into her hand in a thin thread of light. With her left hand she is reaching past the player's shoulder to fasten a fine white-metal chain around their neck: the violet stone, her stone, its blue veins lit, now leaving her chest for the player's, the only violet in the frame. Her expression is the tenderest thing on the peninsula: eyes half-closed, the faint smile of someone receiving the one gift she could never take. In the wings every reflected face has gone still and peaceful, all of them looking at the player. The light between the two figures is warm and gold; the sky behind is the first real blue of dawn breaking. Painterly realism, cinematic lighting, extreme detail on skin, metal, glass and the stone. No nudity, no halo, no feathers, no text.

> **REFUSE.** Same stage, same woman, and the calm has cracked. Illyria's right hand is still out, palm up, and it is closing on nothing — fingers half-curled, empty, the exact centre of the image. Her face is still beautiful and almost still, but the violet irises have gone wide, the blue veins in them burning brighter, and the mouth has tightened by one degree, the first anger she has felt in centuries. Everything behind her has changed: the vast fractured-mirror wings are shattering outward in slow motion, hundreds of glass shards flying, and every shard's reflected face is screaming — and the largest shards are already landing on the platform and standing up as glass-and-light figures of the dead, a dozen of them, forming a ring. The player stands in the foreground with their back to the viewer, hood down, drawing a weapon from their back, planted on the wet stone. The gold pre-dawn light is gone; the sky is cold white and steel-blue; the violet stone at her chest is the one saturated colour in the frame and its veins are glowing. Painterly realism, cinematic boss-fight key art, extreme detail on glass, metal and skin. No nudity, no halo, no feathers, no text.

> **MERGE.** Same stage, and the two figures are becoming one. Illyria and the hooded player, seen from the side, chest to chest at the centre of the platform; two spheres of light — one soft white from the player's chest, one blinding platinum from hers — have met between them and are fusing into a single sun-bright core. From the point where they touch, both bodies are turning to the same substance: fractured mirror-glass and light climbing up her from the waist and up the player from the shoulders, their edges dissolving into each other so it is no longer clear where one ends. Behind and above them a new shape is already rising out of the fusing light — a colossal figure of glass, light and hundreds of faces, wings that are more like a shattered cathedral window than wings, beautiful and terrible in equal measure, an abomination of the light rather than of flesh. The violet stone hangs at the exact centre of the fusion, the one solid thing in the frame, its blue veins burning white. The platform's Forge-runes are lit for the first time in the whole series. Sky: the dawn has stopped halfway, half gold, half black. Painterly realism, cinematic key art, extreme detail on glass and light. No nudity, no gore, no halo, no feathers, no text.
...
async function main() {
  ...
  if (!existing) {
    ...
    if (apply) {
      ...
      await db.storyRevision.create({ data: { id: randomUUID(), entityType: "ENTRY", entityId: existing.id, action: "UPDATED", actorUserId: actor.id, summary: "Her eyes and her stone are violet, veined with faintly lit stormglass blue — confirmed from the accepted render; every red on the sheet corrected." } });
    }
  } else {
