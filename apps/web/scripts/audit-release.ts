...
import { buildAtlasIntegrityAudit, createFilesystemAtlasArtworkInspector } from "./lib/atlas-integrity";
...
async function main() {
  ...
  for (const finding of atlas.findings) {
    ...
    if (waivers[finding.path]) images.waived.push(`${detail}
             accepted because: ${waivers[finding.path]}`);
  ...
  if (process.argv.includes("--json")) {
    console.log(JSON.stringify({ contract: "habitat-release-audit", contractVersion: 1, result: failed.length === 0 ? "PASS" : "FAIL", checks }, null, 2));
  } else {
    console.log("Habitat release audit — read-only\n");
    for (const entry of checks) {
      console.log(`${entry.failures.length === 0 ? "PASS" : "FAIL"}  ${entry.name}`);
      for (const note of entry.notes) console.log(`      ${note}`);
      for (const item of entry.waived) console.log(`      WAIVED ${item}`);
      for (const failure of entry.failures) console.log(`      ✖ ${failure}`);
    }
    const waivedCount = checks.reduce((total, entry) => total + entry.waived.length, 0);
    console.log(`\n${failed.length === 0 ? "PASS" : "FAIL"} — ${checks.length - failed.length}/${checks.length} checks clear${waivedCount ? `, ${waivedCount} accepted defect${waivedCount === 1 ? "" : "s"}` : ""}.`);
    if (failed.length > 0) console.log("This release must not ship until these are fixed.");
  }
